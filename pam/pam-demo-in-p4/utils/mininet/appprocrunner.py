...
class AppProcess:
    ...
    def start(self):
        ...
        cmd_arg = self.host_conf['cmd']
        #print('*' * 20)
        #print(cmd_arg)
        #print(type(cmd_arg))
        cmd_strlist = [cmd_arg,] if isinstance(cmd_arg, str) or isinstance(cmd_arg, unicode) else cmd_arg
        #print(cmd_strlist)
        #print('#' * 20)

        self.cmds = [self.formatCmd(s) for s in cmd_strlist]

        self.proc_list = []
        for cmd in self.cmds:
            proc = self.host.popen(cmd, stdout=self.stdout_file, shell=True, preexec_fn=os.setpgrp)
            self.proc_list.append(proc)

        print self.host.name, self.cmds
    ...
    def waitForExit(self):
        ...
        for proc in self.proc_list:
            print self.host.name, proc.communicate()
    ...
    def killTree(self):
        for proc in self.proc_list:
            _, stdout, _ = self.run_command('pstree -p %d' % proc.pid)
            descendants = re.findall("\(([^)]*)\)", stdout)

            cmd = 'kill -INT %s' % ' '.join(descendants)
            rc, stdout, _ = self.run_command(cmd)

            print "Sent INT to children of PID %d: `%s`. Return code: %d" % (proc.pid, cmd, rc)

    def kill(self):
        for proc in self.proc_list:        
            if proc.returncode is not None: # already exited
                continue

            self.run_command('pkill -INT -P %d' % proc.pid)
            sleep(0.2)

            rc, _, _ = self.run_command('pkill -0 -P %d' % proc.pid) # check if it's still running
